...
#define MA35H0_DDR_HW_POWER_DOWN	0

/* Macros to read the rk power domain state */
#define MA35H0_CORE_PWR_STATE(state) \
    ((state)->pwr_domain_state[MPIDR_AFFLVL0])
#define MA35H0_CLUSTER_PWR_STATE(state) \
    ((state)->pwr_domain_state[MPIDR_AFFLVL1])
#define MA35H0_SYSTEM_PWR_STATE(state) \
((state)->pwr_domain_state[PLAT_MAX_PWR_LVL])

static uintptr_t ma35h0_sec_entrypoint;
...
static __inline void ma35h0_LockReg(void)
{
	mmio_write_32(0x404601A0, 0);
}

#if MA35H0_DDR_HW_POWER_DOWN
// Q ch
void ma35h0_ddr_hw_pd(void)
{
	int DDR_QCH_BPPORT0;
	int DDR_QCH_BPPORT1;
	int DDR_QCH_BPPORT2;
...
void ma35h0_deep_power_down(void)
{
	ma35h0_UnlockReg();
...
void ma35h0_deep_power_down_sw(void)
{
	ma35h0_UnlockReg();
...
static void ma35h0_cpu_standby(plat_local_state_t cpu_state)
{
...
static int ma35h0_pwr_domain_on(u_register_t mpidr)
{

	if (mpidr == 1)
	{
		mmio_write_32(SYS_BASE + CA35WRBADR2, ma35h0_sec_entrypoint);
		sev();
	}
	else
	{
		mmio_write_32(SYS_BASE + CA35WRBPAR1, 0x7761726D);
		mmio_write_32(SYS_BASE + CA35WRBADR1, ma35h0_sec_entrypoint);
	}

	return PSCI_E_SUCCESS;
}

static void ma35h0_pwr_domain_off(const psci_power_state_t *target_state)
{

}

static void ma35h0_pwr_domain_suspend(const psci_power_state_t *target_state)
{
	unsigned int reg;
	...
	mmio_write_32(SYS_BASE + CA35WRBPAR1, 0x7761726D);
#if MA35H0_DDR_HW_POWER_DOWN
	ma35h0_deep_power_down();
#else
	ma35h0_deep_power_down_sw();
#endif
}

static void ma35h0_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	plat_arm_gic_init();
...
int ma35h0_validate_ns_entrypoint(uintptr_t ns_entrypoint)
{
	return PSCI_E_SUCCESS;
}


static void ma35h0_pwr_domain_suspend_finish(const
...
/*******************************************************************************
 * MA35H0 handlers to shutdown/reboot the system
 ******************************************************************************/
...
static int ma35h0_validate_power_state(unsigned int power_state,
...
void ma35h0_get_sys_suspend_power_state(psci_power_state_t
        *req_state)
{
	int i;
...
void __dead2 ma35h0_pwr_domain_pwr_down_wfi(const psci_power_state_t * target_state)
{
	u_register_t scr;
...
plat_psci_ops_t plat_arm_psci_pm_ops = {
	.cpu_standby = ma35h0_cpu_standby,
	.pwr_domain_on = ma35h0_pwr_domain_on,
	.pwr_domain_off = ma35h0_pwr_domain_off,
	.validate_ns_entrypoint = ma35h0_validate_ns_entrypoint,
	.pwr_domain_suspend = ma35h0_pwr_domain_suspend,
	.pwr_domain_on_finish = ma35h0_pwr_domain_on_finish,
	.pwr_domain_suspend_finish = ma35h0_pwr_domain_suspend_finish,
	.system_off = ma35h0_system_off,
	.system_reset = ma35h0_system_reset,
	.validate_power_state = ma35h0_validate_power_state,
	.pwr_domain_pwr_down_wfi = ma35h0_pwr_domain_pwr_down_wfi,
	.get_sys_suspend_power_state = ma35h0_get_sys_suspend_power_state,
};
...
{
	ma35h0_sec_entrypoint = sec_entrypoint;
