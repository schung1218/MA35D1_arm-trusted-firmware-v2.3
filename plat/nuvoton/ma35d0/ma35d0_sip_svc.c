...
#include <ma35d0_sip_svc.h>
#include <drivers/nuvoton/ma35d0_pmic.h>
#include "ma35d0_private.h"
#include "include/ma35d0_crypto.h"

#pragma weak ma35d0_plat_sip_handler
...
uintptr_t ma35d0_plat_sip_handler(uint32_t smc_fid,
					u_register_t x1,
					u_register_t x2,
					u_register_t x3,
					u_register_t x4,
 ...
 */
static int32_t ma35d0_set_cpu_clock(int cpu_freq)
{
	uint64_t timeout;
			  ...
			  u_register_t flags)
{
	...
	case SIP_SVC_PMIC:
		volt = (uint32_t)x2;
		if (volt == 0)
			volt = ma35d0_get_pmic(x1);
		else {
			if (volt != ma35d0_get_pmic(x1))
				ma35d0_set_pmic(x1, x2);
		}
	...
	case SIP_CPU_CLK:
		...
		ret = ma35d0_set_cpu_clock(CPU_CLK);
		if (ret == 1) {
	...
	case SIP_LOW_SPEED:
		if ((uint32_t)x1 == 0) {
			...
			ma35d0_set_cpu_clock(CPU_PLL_500);
			...
			ma35d0_set_cpu_clock(CPU_PLL_125);
			/* un-lock */
			do {
				mmio_write_32((TSI_SYS_BASE + 0x100), 0x59);
				mmio_write_32((TSI_SYS_BASE + 0x100), 0x16);
	...
	default:
		mmio_write_32(SYS_RLKTZS, 0);
		return ma35d0_plat_sip_handler(smc_fid, x1, x2, x3, x4,
			cookie, handle, flags);
	};
