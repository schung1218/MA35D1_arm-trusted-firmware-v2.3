...
#include <drivers/nuvoton/ma35h0_nand.h>

#define SZ_128M		128U

struct ma35h0_nand_info {
	int pages_per_block;
	int page_size;
	int oob_size;
	int max_bit_corr;
...
struct ma35h0_nand_info ma35h0_nand;
...
/*********************************************************/
int ma35h0_nand_wait_ready(unsigned long delay)
{
#if 0
	uint64_t timeout;
...
int ma35h0_nand_reset(void)
{
	...
	if (ma35h0_nand_wait_ready(1))
		return -ETIMEDOUT;
	else
		return 0;
}

static void ma35h0_nand_read_oob(struct ma35h0_nand_info *nand, unsigned int page)
{
	int volatile i;
	unsigned char *ptr;
...
static void ma35h0_correct_data(unsigned char ucFieidIndex, unsigned char ucErrorCnt, unsigned char* pDAddr)
{
...
static int ma35h0_nand_read_page(struct ma35h0_nand_info *nand, unsigned int page, uintptr_t buffer)
{
	...
	if (nand->max_bit_corr != 0)    /* ECC_EN */
		ma35h0_nand_read_oob(nand, page);
				...
				for (j=0; j<uLoop; j++) {
					...
					for (i=1; i<5; i++) {
						...
						if ((uF1_status & 0x3)==0x01) {
							...
							ma35h0_correct_data(j*4+i, uErrorCnt, (unsigned char*)(long)buffer);
							break;
...
static int ma35h0_nand_read_pages(struct ma35h0_nand_info *nand, uintptr_t buf, int page_start, int page_count)
{
	while (page_count > 0)
	{
		if (ma35h0_nand_read_page(nand, page_start, buf)) {
			return -EBADMSG;
		}
		buf += nand->page_size;
		page_count--;
...
static int ma35h0_block_isbad(struct ma35h0_nand_info *nand, unsigned int pba)
{
	...
	ma35h0_nand_read_oob(nand, page+1);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */

	ma35h0_nand_read_oob(nand, page+nand->pages_per_block-1);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */

	ma35h0_nand_read_oob(nand, page+nand->pages_per_block-2);
	data = mmio_read_32(REG_NANDRA0) & 0xff;
	if (data != 0xFF)
		return 1;   /* invalid */
...
static size_t parse_nand_read(struct ma35h0_nand_info *nand, int lba, uintptr_t buf, size_t size)
{
	...
	while (pages_to_read) {
		ret = ma35h0_block_isbad(nand, block);
		if (ret) {
			block++;
			if ((--block_count) <= 0)
				goto out;
		...
		ret = ma35h0_nand_read_pages(nand, p, block * pages_per_block + page, page_count);
		if (ret)
			goto out;
...
int ma35h0_nand_ctrl_init(void)
{
	/* enable nand clock */
...
static void ma35h0_nand_setup(struct ma35h0_nand_info *nand)
{
	unsigned int reg = 0x04910090; // REG_NANDCTL default value
	int node, count;

	ma35h0_nand_ctrl_init();
	...
	node = fdt_node_offset_by_compatible(fdt, -1, "nuvoton,ma35h0-nand");
	if (node < 0) {
		WARN("The compatible property `nuvoton,ma35h0-nand` not found\n");
	}
...
static size_t ma35h0_nand_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	count = parse_nand_read(&ma35h0_nand, lba, buf, size);
...
static struct io_block_dev_spec nand_dev_spec = {
	.ops = {
		.read = ma35h0_nand_read,
	},
	/* fill .block_size at run-time */
};

int ma35h0_nand_init(struct io_block_dev_spec **block_dev_spec, long *offset)
{
	ma35h0_nand_setup(&ma35h0_nand);
	*offset = ma35h0_nand.offset;

	nand_dev_spec.block_size = ma35h0_nand.page_size;
