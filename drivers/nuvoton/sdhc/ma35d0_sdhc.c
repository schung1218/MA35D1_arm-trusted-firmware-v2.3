...
#include <drivers/nuvoton/ma35d0_sdhc.h>
...
static int ma35d0_sdhc_hw_init(struct mmc *mmc)
{
	struct mmc_cmd cmd;
	int err;
...
static void ma35d0_sdhc_setup(struct mmc *mmc)
{
	int node;
	...
	ma35d0_sdhc_hw_init(mmc);
}

static size_t ma35d0_sdhc_read(int lba, uintptr_t buf, size_t size)
{
	size_t count = 0;

	inv_dcache_range(buf, size);

	if (sdh_read_blocks(&ma35d0_mmc, lba, (void *)buf, size/512) == 0)
		count = size;
...
static struct io_block_dev_spec sdhc_dev_spec = {
	.ops = {
		.read = ma35d0_sdhc_read,
	},
	.block_size = 512,
};

int ma35d0_sdhc_init(struct io_block_dev_spec **block_dev_spec,
				   long *offset, int sdhc)
{
	if (sdhc == 0) {
		...
		ma35d0_mmc.base = SDH1_BASE;
	}

	ma35d0_sdhc_setup(&ma35d0_mmc);
	*offset = ma35d0_mmc.offset;
