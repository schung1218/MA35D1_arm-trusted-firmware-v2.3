...
#if (MA35D0_PMIC == PMIC_IP6103)
#define DEVICE_ADDR 0x60
#endif

#if (MA35D0_PMIC == PMIC_DIALOG)
#define DEVICE_ADDR 0xB0
#endif

#if (MA35D0_PMIC == PMIC_APW7704F)
#define DEVICE_ADDR (0x25 << 1)
#endif

#if (MA35D0_PMIC == PMIC_NO)
...
unsigned int ma35d0_write_i2c_data(unsigned int u32Addr, unsigned int u32Data)
{
...
unsigned int ma35d0_read_i2c_data(unsigned int u32Addr, unsigned int *u32Data)
{
	unsigned int I2C_TIME_OUT_COUNT = 0x200000;
	unsigned int u32Status;
	unsigned int u32time_out = 0;
...
unsigned int ma35d0_read_pmic_data(
	...
	unsigned int *u32Data)
{
	...
	while (j-- > 0) {
		if (ma35d0_read_i2c_data(u32Addr, u32Data) == 2) {
			break;
		}
	}
...
unsigned int ma35d0_write_pmic_data(unsigned int u32Addr, unsigned int u32Data)
{
	int j = RETRY_COUNT;

	while (j-- > 0) {
		if (ma35d0_write_i2c_data(u32Addr, u32Data) == 1) {
			break;
		}
	}
...
void ma35d0_i2c0_init(unsigned int sys_clk)
{
...
int ma35d0_set_pmic(int type, int vol)
{
	...
	if (pmicIsInit == 0) {
		ma35d0_i2c0_init(pmic_clk);
		pmicIsInit = 1;
		ma35d0_write_pmic_data(0x2e, 0x00);
		ma35d0_write_pmic_data(0x35, 0x00);
	}
	...
	if (type == VOL_CPU) {
		...
		case VOL_1_00:
			INFO("IP6103 1.00V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x24);
			break;
		case VOL_1_10:
			INFO("IP6103 1.10V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x28);
			break;
		case VOL_1_15:
			INFO("IP6103 1.15V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x2c);
			break;
		case VOL_1_18:
			INFO("IP6103 1.175V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x2e);
			break;
		case VOL_1_19:
			INFO("IP6103 1.1875V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x2f);
			break;
		case VOL_1_20:
			INFO("IP6103 1.20V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x30);
			break;
		case VOL_1_21:
			INFO("IP6103 1.2125V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x31);
			break;
		case VOL_1_22:
			INFO("IP6103 1.225V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x32);
			break;
		case VOL_1_23:
		case VOL_1_24:
			INFO("IP6103 1.2375V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x33);
			break;
		case VOL_1_25:
			INFO("IP6103 1.25V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x34);
			break;
		case VOL_1_26:
			INFO("IP6103 1.2625V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x35);
			break;
		case VOL_1_27:
			INFO("IP6103 1.275V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x36);
			break;
		case VOL_1_28:
		case VOL_1_29:
			INFO("IP6103 1.29V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x37);
			break;
		case VOL_1_30:
			INFO("IP6103 1.30V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x38);
			break;
		case VOL_1_32:
			INFO("IP6103 1.32V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x3a);
			break;
		case VOL_1_34:
			INFO("IP6103 1.34V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x3c);
			break;
		case VOL_1_36:
			INFO("IP6103 1.36V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x3e);
			break;
		default:
		...
		case VOL_3_30:
			INFO("IP6103 SD 3.3V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x68);
			break;
		default:
			ERROR("Not support voltage!\n");
			ret=-1;
		...
		reg0 = 0x4C;
		ma35d0_read_pmic_data(reg1, &temp);
		switch (vol)
		{
		case VOL_1_00:
			INFO("IP6103 LDO5 1.00V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x0C);
			ma35d0_write_pmic_data(reg1,temp|0x20);
			break;
		case VOL_1_20:
			INFO("IP6103 LDO5 1.20V\n");
			ret = ma35d0_write_pmic_data(reg0, 0x14);
			ma35d0_write_pmic_data(reg1,temp|0x20);
			break;
		case VOL_1_80:
		...
		case VOL_2_50:
			...
			ret = ma35d0_write_pmic_data(reg0, 0x48);
			ma35d0_write_pmic_data(reg1,temp|0x20);
			break;
		case VOL_3_30:
		...
		case VOL_1_00:
			...
			ret = ma35d0_write_pmic_data(reg0, 0x0C);
			ma35d0_write_pmic_data(reg1,temp|0x40);
			break;
		case VOL_1_20:
...
int ma35d0_set_pmic(int type, int vol)
{
	...
	if (type == VOL_CPU) {
		...
		reg = 0x32;
	 	if (vol == VOL_1_80) {
			INFO("1.8V\n");
			ret = ma35d0_write_pmic_data(reg, 0x80);
		} else if (vol == VOL_3_30) {
			INFO("3.3V\n");
			ret = ma35d0_write_pmic_data(reg, 0x00);
		} else {
			ERROR("Not support voltage!\n");
			ret = -1;
		}
...
#elif (MA35D0_PMIC == PMIC_APW7704F)
int ma35d0_set_pmic(int type, int vol)
{
	...
	case VOL_1_10:
		INFO("APW7704F 1.11V\n");
		ret = ma35d0_write_pmic_data(reg, 0x32);
		break;
	case VOL_1_18:
		INFO("APW7704F 1.17V\n");
		ret = ma35d0_write_pmic_data(reg, 0x32);
	...
	case VOL_1_22:
		INFO("APW7704F 1.20V\n");
		ret = ma35d0_write_pmic_data(reg, 0x33);
		break;
	case VOL_1_23:
	case VOL_1_24:
		INFO("APW7704F 1.23V\n");
		ret = ma35d0_write_pmic_data(reg, 0x34);
		break;
	case VOL_1_25:
	case VOL_1_26:
	case VOL_1_27:
		INFO("APW7704F 1.26V\n");
		ret = ma35d0_write_pmic_data(reg, 0x35);
		break;
	case VOL_1_28:
	case VOL_1_29:
		INFO("APW7704F 1.29V\n");
		ret = ma35d0_write_pmic_data(reg, 0x36);
		break;
